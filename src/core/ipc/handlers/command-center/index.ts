...
interface FileMetadata {
  ...
  lastModified: Date;
  extension: string;
  type: 'text'; // Assuming only text files are read based on current logic
  truncated: boolean;
}
...
export function registerCommandCenterHandlers(): void {
  ...
  ipcMain.handle(CommandCenterEvents.OPEN_DIALOG, async (_, dialogType: string, data: object): Promise<IpcResponse<null>> => {
    try {
      setDialogState(true);
  ...
  ipcMain.handle(CommandCenterEvents.READ_FILE, async (_event: IpcMainInvokeEvent, options: string | FileReadOptions): Promise<IpcResponse<{ content: string, metadata: FileMetadata }>> => {
    try {
